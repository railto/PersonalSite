...
<main class="sm:container sm:mx-auto sm:max-w-lg sm:mt-10">
    <div class="flex">
        <div class="w-full">
            ...
            @if (session('status'))
            <div class="text-sm text-green-700 bg-green-100 px-5 py-6 sm:border sm:border-green-400 sm:mb-6"
                role="alert">
                {{ session('status') }}
            </div>
            @endif
            ...
            <section class="flex flex-col break-words bg-white sm:border-1 sm:shadow-sm sm:shadow-lg">
                ...
                <form class="w-full px-6 space-y-6 sm:px-10 sm:space-y-8" method="POST" action="{{ route('password.email') }}">
                    ...
                    <div class="flex flex-wrap justify-center items-center space-y-6 pb-6 sm:pb-10 sm:space-y-0 sm:justify-between">
                        <button type="submit"
                        class="w-full select-none font-bold whitespace-no-wrap p-3 text-base leading-normal no-underline text-gray-100 bg-indigo-500 hover:bg-indigo-700 sm:w-auto sm:px-4 sm:order-1">
                            Send Password Reset Link
                        </button>

                        <p class="mt-4 text-xs text-indigo-500 hover:text-indigo-700 whitespace-no-wrap no-underline hover:underline sm:text-sm sm:order-0 sm:m-0">
